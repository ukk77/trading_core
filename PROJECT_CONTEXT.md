...
> `trading_core` · 9 files (python×9) · 14 classes · 22 functions · ~1,780 source lines
...
- `«dataclass» class LossGuardConfig`  — Configuration for the loss guard (enabled, max_consecutive_losses, cooloff_bars).
- `class LossGuard`  — Per-ticker consecutive loss tracker with cool-off period.
  methods: `from_config`, `record_loss`, `record_win`, `tick`, `is_blocked`, `get_streak`, `get_cooloff_remaining`, `reset`, `reset_all`

**`regime_params.py`** `python`
> Regime-adaptive parameter selector — reads the current market regime and returns strategy-specific parameter multipliers.
- `«dataclass» class RegimeAdjustment`  — Holds per-strategy parameter multipliers (entry_threshold_mult, position_size_mult, stop_width_mult, adx_threshold_mult, sentiment_strictness_mult, max_risk_score, regime_detected).
- `def get_regime_adjustments(strategy: str, regime_override: Optional) → RegimeAdjustment`  — Return regime-specific parameter adjustments for a strategy.
- `def _read_latest_regime_from_db() → Optional`  — Read the most recent regime from the harness regime_log table.